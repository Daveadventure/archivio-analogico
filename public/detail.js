...
function chip(label){
  return `<span class="chip" style="display:inline-block;border:1px solid var(--line);padding:6px 10px;border-radius:999px;background:rgba(255,255,255,.55);margin:4px 6px 0 0;font-size:12px;">${esc(label)}</span>`;
    ...
    if (btn && embed) {
      btn.addEventListener("click", () => {
        ...
        btn.disabled = true;
        btn.textContent = "Spotify pronto ✅";
      });
    }
...
(async function init(){
  ...
  try{
    ...
    const cover = (data.images||[]).find(i => i && i.type === "primary")?.uri || data.images?.[0]?.uri || data.cover_image || data.thumb || data.basic_information?.cover_image || data.basic_information?.thumb || "";
    ...
    // ===== Vintage meta badges (safe string, no nested templates) =====
    ...
    const isMono = desc.some(x => x.includes("mono"));
    const isStereo = desc.some(x => x.includes("stereo"));

    const badges = [];
    if (country) badges.push('<span class="badge badgePress">' + esc(country) + ' PRESS</span>');
    if (catnos.length) badges.push('<span class="badge">CAT: ' + esc(catnos[0]) + '</span>');
    if (isMono) badges.push('<span class="badge badgeMono">MONO</span>');
    else if (isStereo) badges.push('<span class="badge badgeStereo">STEREO</span>');
